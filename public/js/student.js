...
function showFiles(input) {
    ...
    Array.from(input.files).forEach(file => {
        ...
        item.innerHTML = `<i class="fas fa-file-alt"></i> ${file.name} <span style="color:#aaa;margin-left:auto;font-size:12px">${(file.size/1024).toFixed(1)} KB</span>`;
...
fileDrop.addEventListener("dragover", e => { e.preventDefault(); fileDrop.style.borderColor="#0057D9"; });
...
async function cargarHistorial() {
    try {
        ...
        renderHistorial(misSolicitudes, respuestas);
        ...
        tabla.innerHTML += `
            <tr>
                ...
                <td style="color:${respuesta ? "#2E7D32" : "#999"}">
                    ${respuesta ? `<i class="fas fa-comment-dots" style="margin-right:5px"></i>${respuesta.message}` : "Sin respuesta aún"}
                </td>
            </tr>
        `;
    });
...
/* Sobrescribir cargarHistorial para guardar cache */
const _cargarHistorial = cargarHistorial;
async function cargarHistorial() {
    try {
...
async function buscarSolicitud() {
    ...
    try {
        ...
        if (solicitud.status_id == 2) {
            labelEstado = "label-process";
            iconEstado  = "fas fa-spinner";
            textoEstado = "En proceso";
        } else if (solicitud.status_id == 3) {
            labelEstado = "label-resolved";
            iconEstado  = "fas fa-check-circle";
            ...
            <div class="track-response ${respuesta ? "has-response" : "no-response"}">
                ...
                <div>
                    ...
                    <p class="track-response-text">
                        ${respuesta ? respuesta.message : "Tu solicitud está siendo revisada. Recibirás respuesta en 3 a 4 días hábiles."}
                    </p>
                </div>
            </div>
        `;
...
document.getElementById("trackingId")
    .addEventListener("keydown", e => {
        if (e.key === "Enter") buscarSolicitud();
    });
